<template>
  <header class="flex shadow-lg py-4 px-4 sm:px-10 bg-white font-[sans-serif] min-h-[70px] tracking-wide relative z-50">
    <div class="flex flex-wrap items-center justify-between gap-4 w-full">
      ...
      <!-- Collapsible Menu -->
      <div :class="{'block': isMenuOpen, 'hidden': !isMenuOpen, 'lg:block': true}"
           class="max-lg:w-full max-lg:fixed max-lg:before:fixed max-lg:before:bg-black max-lg:before:opacity-50 max-lg:before:inset-0 max-lg:before:z-50">
        
        <!-- Close Button -->
        <button @click="toggleMenu" class="lg:hidden fixed top-2 right-4 z-[100] rounded-full bg-white p-3">
